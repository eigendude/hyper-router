...
impl<'r> Router<'r> {
    ...
    pub fn find_handler_with_defaults(&self, request: &Request) -> &'r &Box<Handler> {
    ...
    /// status codes.
    pub fn find_handler(&self, request: &Request) -> HttpResult<&'r Box<Handler>> {
        if let AbsolutePath(request_path) = request.uri.clone() {
            let matching_routes = self.find_matching_routes(&request_path);
            match matching_routes.len() {
    ...
    fn find_for_method(&self, routes: &Vec<&Route>, method: &Method) -> Option<&'r Box<Handler>> {
